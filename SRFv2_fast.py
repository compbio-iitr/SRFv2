...
import re
from html_file_fast import html

#import parallel_winspectra
import prettifyoutput_fast
...
def get_inputs():
    ...
    cnt = len(inputs)
    if cnt < 6:
        print('USAGE: SRFv2_fast.py <fasta sequence file> <min mer> <max mer> <%%match> <output directory> [options]\n\n')
        # TODO add outputs
        kill()
    return inputs
...
def win_fourier_spectra(argument):
    ...
    FWO.write('# Peak Region Mers %d were found in the regions\n' % (mer))
    index = np.array(index)
    peak = np.array(peak)
    maximum_indexes = np.where(peak == peak.max())[0]
    maximum_indexes.sort()
    for mindex in maximum_indexes:
        start = max(1+i14*19000,index[mindex] - window)
        stop = min(length_of_subseq+i14*19000,index[mindex] + window)
        FWO.write('# P %d %d\n'%(start, stop))             
    FWO.close()
...
def win_stransform_spectra(argument):
    ...
    for ii in range(0,length_of_subseq,10):
        tickoff = 0
        peak_values = []
        plot_values = []
        iii = ii+1 + i14*19000
...
def patterns(mer,pattern_genome):
    
    patt = ['xxx']
    
    length_of_pattern = len(pattern_genome)
    
    for i6 in range(length_of_pattern):
        string = pattern_genome[i6:i6+mer]
        if len(string) < mer:
            break
        s = 0
        i7 = 0
...
def pattern_finder_v2(argument):
    genome = argument[0]
    outfile = argument[1]
    mer = argument[2]
    rstart = argument[3]
    ...
    with open(win_spectra_file, 'r') as file:
        for line in file:
            ...
            if not line.startswith('#'):
                ...
                max_y = max(max_y, y)
            if line.startswith('#'):
                codw = re.split(r'\s+', line)
                if '#' in line and line.index('#') == 0 and codw[1] == 'W':
                    check_window = True
                count = count + 1
    plt.figure()
...
def parallel_pattern_v2(args):
    ...
    pool.map(pattern_finder_v2, args)
    pool.close()
    pool.join()
...
if __name__ == '__main__':
    ...
    if length_of_sequence > 25000:
        ...
        for i14 in range(0, length_of_sequence, 19000):
            ...
            for mer in mers_length:
                ...
                if os.path.exists(output_dir + '/fourier_wspec%d_%s.file'%(filenum, mer)):
                    with open(output_dir + '/fourier_wspec%d_%s.file'%(filenum, mer),'r') as FG:
                        for lsg in FG:
                            ...
                            if '#' in lsg and lsg.index('#') == 0 and codw[1] == 'W':
                                regions.append([int(codw[2]),int(codw[3])])
                                check_window = True
                            elif len(regions) > 0 and check_window:
                                if '#' in lsg and lsg.index('#') == 0 and codw[1] == 'P':
                                    peak_regions.append([int(codw[2]),int(codw[3])])
                check_window = False
                ...
                if len(regions) > 0:
                    ...
                    peak_regions = np.array(peak_regions)
                    
                    regions = np.sort(regions,axis=0)
                    peak_regions = np.sort(peak_regions,axis=0)
                    
                    merged_regions = []
                    merged_peak_regions = []
    
                    merge = np.copy(regions)
                    merge_peak = np.copy(peak_regions)
                    ...
                    for i in range(1,merge_peak.shape[0]):
                        for j in range(0,i):
                        
                            if merge_peak[i,0]>=merge_peak[j,0] and merge_peak[i,0]<=merge_peak[j,1]+100:
                                merge_peak[i,0] = merge_peak[j,0] 
                                if merge_peak[i,1] > merge_peak[j,1]:
                                    merge_peak[j,1] = merge_peak[i,1]
                  
                
                    uniq_initial = np.unique(merge_peak[:,0])
                    uniq_final = np.unique(merge_peak[:,1])
                
                    for i in range(uniq_initial.shape[0]):
                        merged_peak_regions.append([uniq_initial[i],uniq_final[i]])
                    
                    merged_peak_regions = np.array(merged_peak_regions)
                    
                    final_patterns = []
                        
                    for i in range(merged_peak_regions.shape[0]):
                        peak_dif = int(merged_peak_regions[i,1]) - (int(merged_peak_regions[i,0])-1)
                        peak_genome = genome[(int(merged_peak_regions[i,0])-1):(int(merged_peak_regions[i,0])-1)+peak_dif]
                        patt = patterns(mer,peak_genome)
                        for p in patt:
                    ...
                    for i in range(merged_regions.shape[0]):
                        ...
                        parallel_arg = [string2,merfile,int(mer),initreg-i14,int(merged_regions[i,0]),final_patterns]
            ...
            prettifyoutput_fast.pat2json(output_dir + '/pat%d.file'%filenum, output_dir + '/data%d.json'%filenum,output_dir + '/modified%d.json'%filenum,output_dir+'/SRFv2_results%d.file'%(filenum),filenum, 1, len(genome1), initreg, initreg + len(genome1)-1)
        ...
        for mer in mers_length:
            ...
            if len(regions) > 0 and len(peak_regions) > 0:
                ...
                for i in range(merged_regions.shape[0]):
                    ...
                    string2 = genome[(int(merged_regions[i,0])-1):(int(merged_regions[i,0])-1)+dif]
                    merfile = output_dir + '/subpat%d-%s.res' % (filenum, mer)
                    parallel_arg = [string2,merfile,int(mer),1,int(merged_regions[i,0]),final_patterns]
                    final_regions.append([int(merged_regions[i,0]),int(merged_regions[i,1]),int(mer)])
                    pattern_args.append(parallel_arg)
        final_regions = np.array(final_regions)
        ...
        with open(output_dir + '/pat%d.file'%filenum,'a') as FPO:
            for j in range(len(final_mers)):
                with open(output_dir+'/subpat%d-%s.res'%(filenum,final_mers[j]),'r') as sub:
                    ...
                    del pattern, pattern_full, pattern_split
        prettifyoutput_fast.pat2json(output_dir + '/pat%d.file'%filenum, output_dir + '/data%d.json'%filenum,output_dir + '/modified%d.json'%filenum,output_dir+'/SRFv2_results%d.file'%filenum,filenum, 1, len(genome),1, len(genome))
        ...
        parallel_pattern_v2(pattern_args)
        mers.sort()
        list_mers = glob(output_dir+'/subpat%d-'%(filenum)+'*.res')
        final_mers = []
        for l in list_mers:
    ...
    with open(output_dir + '/completed', 'w') as compl:
        compl.write('1')
    with open(output_dir + '/fast.txt', 'w') as compl:
        compl.write('1')
